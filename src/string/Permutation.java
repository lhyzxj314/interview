...
public class Permutation {
	public ArrayList<String> permutation(String str) {
		...
		for (int i = 0; i < sb.length(); i++) {
			...
			Set<StringBuffer> strsBuffer = permution(head, tail);
	...
	public Set<StringBuffer> permution(StringBuffer head, StringBuffer tail) {
		Set<StringBuffer> res = new HashSet<StringBuffer>();
		if (tail.length() == 0) {
			res.add(head);
			return res;
		}
		...
		for (int i = 0; i < tail.length(); i++) {
			...
			Set<StringBuffer> tails = permution(tTead, tTail);
